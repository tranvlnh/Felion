...
internal static class EvaluationMessageFactory
{
    public static InteractionMessageProperties Created(EvaluationPeriodDto period) =>
        Message(
            ...
            string.Format(
                ...
                "**Name:** {0}\n**Status:** {1}\n**Opened:** {2}\n\nCandidate dùng /evaluation peer; mentor dùng /evaluation mentor.",
                period.Name,
                period.Status,
    ...
    public static InteractionMessageProperties Current(EvaluationStatusDto status)
    {
        ...
        return Message(
            "Current Evaluation — " + status.Period.Name,
            string.Format(
                CultureInfo.InvariantCulture,
                "**Status:** {0}\n**Opened:** {1}\n\n**Peer progress:** {2}/{3}\n**Mentor progress:** {4}/{5}",
    ...
    public static InteractionMessageProperties Targets(EvaluationTargetListDto targetList, bool mentor)
    {
        ...
        if (options.Length > 0)
        {
            ...
            components.Add(
                new StringMenuProperties(customId, options)
                    .WithPlaceholder(mentor ? "Chọn candidate cần đánh giá" : "Chọn candidate trong team của bạn"));
        }
        ...
        {
            description += "\nChỉ 25 candidate đầu tiên có thể chọn trong một Discord select menu.";
        }
